...
const SalonChatPage = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      await chatService.sendMessageStream(currentMessages, handleChunk, 'reine_mere', currentSessionId || undefined);
    } catch (error) {
      ...
      const fallbackMessage: Message = {
        id: Date.now().toString(),
        content: "Pardonne-moi, ma Queen. Il semble que je sois temporairement indisponible. Mon énergie mystique a besoin de se régénérer. Reviens bientôt, et nous pourrons reprendre notre rituel sacré. En attendant, tu peux explorer tes cartes ou faire le quiz pour découvrir ton type de Queen ! ✨👑",
        isUser: false,
        timestamp: new Date()
      };
                        ...
                        <div
                          className={`rounded-2xl px-4 py-2 font-raleway text-sm md:text-base whitespace-pre-line break-words max-w-[80vw] md:max-w-lg
                            ${message.isUser
                              ...
                              : 'bg-[#d6ae60] text-cabinet-ink border-2 border-royal-gold/30 shadow-royal'}
                          `}
                        >
                          {message.content}
